...
class _LandingViewState extends State<LandingView> with WidgetsBindingObserver {
  ...
  Future<void> onsignalSetup() async {
    ...
    try {
      if (await aliyunPush.initPush(
              appKey: Apis.aliyueApiKey, appSecret: Apis.aliyueAppSecret) ==
          false) {
        var deviceToken = await aliyunPush
            .initPush(
                appKey: Apis.aliyueApiKey, appSecret: Apis.aliyueAppSecret)
            .then((value) {
          return value;
      ...
      void onAliyunNotificationClicked(Map<String, dynamic> data) async {
        try {
          ...
          if (data['type'] == 'MESSAGE') {
            ...
            HelpRequest request =
                HelpRequest.fromJson(jsonDecode(data['data']));
            await kOverlayWithAsync(asyncFunction: () async {
              await Future.delayed(const Duration(milliseconds: 500));
            });
          }
