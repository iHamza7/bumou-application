...
class ChatController extends GetxController {
  ...
  var replyMessage = Rx<ChatMessage?>(null);
  ...
  void sendMessage(Map<String, dynamic> message,
    ...
    try {
      // Check if there is a reply and set the replyToMessageId
      if (replyMessage.value != null) {
        message['replyToMessageId'] = replyMessage.value?.id;
      }
  ...
  //       if (data['message']['senderId'] == LocalStorage.getUserId) {
  //         log("Message sent by the user, no notification will be shown.");
  //         return; // Do not show notification if the sender is the current user
  //       }
  ...
  //       _showNotificationForNewMessage(data);
  //     });
  //   } catch (e) {
  //     log("-chat- Error: $e");
  //   }
  ...
  void _showNotificationForNewMessage(Map<String, dynamic> data) async {
    // Log the received data for debugging
    log("Received Data: ${jsonEncode(data)}");

    // Safely access 'sender' and 'message' fields, with fallback values
    String title = (data['chatroom'] != null &&
            data['chatroom']['members'] != null &&
            data['chatroom']['members'][0] != null)
        ? data['chatroom']['members'][0]['firstName'] ?? 'Unknown Sender'
        : 'New Message';

    String body = (data['message'] != null &&
