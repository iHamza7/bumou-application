...
class CommentsController extends GetxController {
  ...
  final RefreshController commentRefreshController =
  ...
  Future<void> addComment(BuildContext context,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        int indexOfMoment = Get.find<MommentController>()
            .moment
        ...
        if (indexOfMoment != -1) {
          ...
          Get.find<MommentController>()
              .moment[indexOfMoment]
              .comments
              ?.insert(0, comment);
  ...
  Future<void> deleteComment(BuildContext context,
    ...
    try {
      var response = await NetworkClient.delete(
        '${Apis.moments}/$postId/comment/$commentId',
      );
      Logger.message('Delete Comment Response: ${response.statusCode}');
      if (response.statusCode == 200) {
        // Remove the comment from the local comments list
        comments.removeWhere((comment) => comment.id == commentId);
        update();

        // Optionally, update the post’s comment count in the MomentsController
        int indexOfMoment = Get.find<MommentController>()
            .moment
            .indexWhere((element) => element.id == postId);
        if (indexOfMoment != -1) {
